...
class AssistantSessionStore:
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        resolved_path = db_path or resolve_app_db_path("assistant_sessions.db")
    ...
    def _init_db(self) -> None:
        with self._get_conn() as conn:
            conn.executescript(
                ...
                CREATE TABLE IF NOT EXISTS assistant_deleted_sessions (
                    raw_conversation_id TEXT PRIMARY KEY,
                    raw_session_id TEXT DEFAULT '',
            ...
            self._ensure_column(conn, "assistant_deleted_sessions", "raw_session_id", "TEXT DEFAULT ''")
            self._ensure_column(conn, "assistant_deleted_sessions", "assistant_session_id", "TEXT DEFAULT ''")
        ...
        if existing_message_id:
            ...
            with self._get_conn() as conn:
                conn.execute(
                    ...
                    SET role = ?, content = ?, content_type = ?, metadata = ?, status = ?, created_at = ?
                    WHERE id = ?
                    """,
                    (
                        role,
                ...
                conn.execute(
                    ...
                    WHERE raw_conversation_id = ?
                    """,
                    (preview, now, raw_conversation_id),
                )
    ...
    def record_deleted_session(
        ...
        *,
        ...
        normalized_raw_id = raw_conversation_id.strip() or (
            f"session:{normalized_session_id}" if normalized_session_id else ""
        )
        ...
        with self._get_conn() as conn:
            conn.execute(
                """
                INSERT INTO assistant_deleted_sessions (
                    raw_conversation_id, raw_session_id, assistant_session_id, deleted_at
                ) VALUES (?, ?, ?, ?)
                ON CONFLICT(raw_conversation_id) DO UPDATE SET
                    raw_session_id = excluded.raw_session_id,
                    assistant_session_id = excluded.assistant_session_id,
                    deleted_at = excluded.deleted_at
                """,
                (normalized_raw_id, normalized_session_id, assistant_session_id or "", now),
            )

    def is_deleted_raw_conversation(
        self,
        raw_conversation_id: str,
        raw_session_id: Optional[str] = None,
    ) -> bool:
        normalized_raw_id = raw_conversation_id.strip()
        normalized_session_id = (raw_session_id or "").strip()
        if not normalized_raw_id and not normalized_session_id:
            return False

        with self._get_conn() as conn:
            if normalized_raw_id:
                row = conn.execute(
                    "SELECT 1 FROM assistant_deleted_sessions WHERE raw_conversation_id = ?",
                    (normalized_raw_id,),
                ).fetchone()
                if row:
                    return True
            ...
            if normalized_session_id:
                row = conn.execute(
                    """
                    SELECT 1 FROM assistant_deleted_sessions
