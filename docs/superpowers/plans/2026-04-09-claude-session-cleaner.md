# Claude Session Cleaner — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a lightweight web tool to browse and batch-delete Claude Code conversation sessions from `~/.claude/history.jsonl` and `~/.claude/file-history/`.

**Architecture:** A single Python script serves an inline HTML/JS/CSS page on localhost. The backend reads `history.jsonl` to build a session list, serves it via JSON API, and handles deletion by rewriting `history.jsonl` and removing `file-history/<sessionId>/` directories. No external dependencies beyond Python stdlib.

**Tech Stack:** Python 3 stdlib (`http.server`, `json`, `pathlib`, `shutil`), vanilla HTML/CSS/JS

---

## Data Model

**`~/.claude/history.jsonl`** — One JSON object per line:
```json
{"display":"消息内容","pastedContents":{},"timestamp":1775675612331,"project":"/Users/huanc/Desktop/ABO","sessionId":"978a1d97-a044-4005-9b1c-7578beea0ade"}
```

**`~/.claude/file-history/<sessionId>/`** — Directories named by UUID, containing conversation data files (e.g. `hash@v2`). Some sessions exist only in `history.jsonl` (no dir), some dirs exist without `history.jsonl` entries.

**Deletion** means:
1. Remove all lines in `history.jsonl` where `sessionId` matches
2. Remove `file-history/<sessionId>/` directory if it exists
...
| File | Responsibility |
|------|---------------|
| `scripts/claude_session_cleaner.py` | Single-file server: HTTP handler + inline HTML template + session CRUD logic |

One file. No config. No dependencies.

---

### Task 1: Python server skeleton with session listing API

**Files:**
- Create: `scripts/claude_session_cleaner.py`

- [ ] **Step 1: Write the failing test — verify the script file exists and is importable**

Create the test file first:

```python
# tests/test_session_cleaner.py
import subprocess, sys, json

def test_server_starts_and_lists_sessions(tmp_path):
    """Write a fake history.jsonl → start server → GET /api/sessions → verify JSON."""
    ...
    file_history = tmp_path / "file-history" / "aaa-111"
    file_history.mkdir(parents=True)
    (file_history / "data@v1").write_text("x")

    # We'll test the parse function directly instead of HTTP (simpler, faster)
    # Import the module
    sys.path.insert(0, str(tmp_path.parent))
    from scripts.claude_session_cleaner import parse_sessions
...
Run: `cd /Users/huanc/Desktop/ABO && python -m pytest tests/test_session_cleaner.py -v`
Expected: FAIL — `ModuleNotFoundError: No module named 'scripts.claude_session_cleaner'`

- [ ] **Step 3: Implement `parse_sessions` and server skeleton**
...
```bash
git add scripts/claude_session_cleaner.py tests/test_session_cleaner.py
git commit -m "feat: add Claude session cleaner web tool"
```

---

### Task 2: Test deletion logic

**Files:**
- Modify: `tests/test_session_cleaner.py`

- [ ] **Step 1: Write the failing test for delete_sessions**

Add to `tests/test_session_cleaner.py`:
...
def test_delete_sessions(tmp_path):
    ...
    from scripts.claude_session_cleaner import delete_sessions
...
```bash
git add tests/test_session_cleaner.py
git commit -m "test: add deletion logic tests for session cleaner"
```
...
- [ ] **Step 1: Run the cleaner against real data**

```bash
cd /Users/huanc/Desktop/ABO && python scripts/claude_session_cleaner.py
```

Expected: Browser opens at `http://127.0.0.1:8234`, shows all 83 sessions sorted by most recent first.

- [ ] **Step 2: Verify the UI**

Check:
- Sessions display with first message text, project path, message count, last active date
- Search filters by display text and project path
- "Select all visible" checkbox works with filtered results
- Delete button shows count and is disabled when nothing selected

- [ ] **Step 3: Test deletion of a known junk session**

Pick a test session (e.g., one of the "are u ok" sessions like `d341a08e`) → select it → click delete → confirm → verify it disappears from the list and the line is gone from `history.jsonl`.

- [ ] **Step 4: Commit (if any fixes were needed)**

```bash
git add scripts/claude_session_cleaner.py
git commit -m "fix: polish session cleaner after manual testing"
```
